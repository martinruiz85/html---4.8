/****** Object:  StoredProcedure [dbo].[Exp_GetDocuments]    Script Date: 03/02/2026 04:29:27 p. m. ******/
...
BEGIN
 ...
 SELECT ISNULL(DI.BuyerId,0) BuyerId,
   ...
   CASE DI.SubAccountId WHEN 0 THEN '-' ELSE CASE WHEN A.[Type]=1 THEN 'Costo de producción' ELSE 'Gasto' END END TypeOfEgress,
   s.Rfc,
   di.Ieps,        
   DI.UUID,
   ...
   DI.Comments,
   RD.Comments AS RejectionReason
 ...
 LEFT JOIN dbo.Exp_RejectedDocument RD WITH(NOLOCK) ON Di.documentId = RD.DocumentId
